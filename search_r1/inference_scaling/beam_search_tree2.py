...
class BeamSearchTree2Generator(BaseInferenceGenerator):
    ...
    def _select_top_candidates(self, expanded_batch, state, original_batch_size):
        """Select top n_candidates unique candidates per original prompt.
        ...
        # Get the responses to check for duplicates
        responses = state.batch.get('responses', None)
        
        selected_indices = []
        
        for prompt_idx in range(original_batch_size):
            prompt_scores = scores_reshaped[prompt_idx]
            
            # Sort indices by score (descending)
            sorted_indices = torch.argsort(prompt_scores, descending=True)
            
            selected_for_prompt = []
            seen_responses = set()
            
            for idx in sorted_indices:
                global_idx = prompt_idx * expanded_per_prompt + idx.item()
                
                # Get the response text if available
                if responses is not None and len(responses) > global_idx:
                    response_text = responses[global_idx]
                ...
                selected_for_prompt.append(global_idx)
                
                # Stop when we have enough unique candidates
                if len(selected_for_prompt) >= self.beam_width:
                    break
            
            # If we don't have enough unique candidates, fill with best remaining
            while len(selected_for_prompt) < self.beam_width and len(selected_for_prompt) < expanded_per_prompt:
                for idx in sorted_indices:
                    global_idx = prompt_idx * expanded_per_prompt + idx.item()
                    if global_idx not in selected_for_prompt:
                        selected_for_prompt.append(global_idx)
                        if len(selected_for_prompt) >= self.beam_width:
                            break
            
            selected_indices.extend(selected_for_prompt[:self.beam_width])
        
        # Convert to tensor
        top_indices_1d = torch.tensor(selected_indices, device=scores.device)
