...
class BeamSearchTreeGenerator(BaseInferenceGenerator):
    ...
    def _select_top_candidates(self, expanded_batch, state, original_batch_size):
        ...
        top_scores, top_indices_2d = torch.topk(scores_reshaped, self.beam_width, dim=1, largest=True)
    ...
    # Improved implementation from beam_search.py
    # def _select_top_candidates(self, expanded_batch, state, original_batch_size):
    #     """Select top n_candidates candidates per original prompt.
