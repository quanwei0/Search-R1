...
def main_task(config):
    ...
    val_metrics, save_dir = trainer._validate()
    pprint(f'Initial validation metrics: {val_metrics}')
    # trainer.logger.log(data=val_metrics, step=0)
    
    # Save val_metrics to save_dir
    if save_dir is not None:
        import json
        import os
        metrics_file = os.path.join(save_dir, 'val_metrics.json')
